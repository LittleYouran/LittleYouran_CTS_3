...
class CpuTurboSchedulerControl {
    ...
    async readFileContent(filePath) {
        try {
            ...
            const result = await this.exec(`cat "${filePath}" 2>/dev/null`);
    ...
    async writeFileContent(filePath, content) {
        try {
            ...
            await this.exec(`echo '${escapedContent}' > "${filePath}" && chmod 644 "${filePath}"`);
    ...
    async saveFile() {
        ...
        try {
            ...
            if (editingFile.name === 'mode.txt') {
                ...
                await this.writeFileContent(editingFile.path, content);
            }
