...
public:
    ...
    void InotifyMain(const char* dir_name, uint32_t mask, const char* watchFileName = nullptr) {
        std::string dirPath;
        std::string fileName;
        if (watchFileName) {
            const char* lastSlash = strrchr(dir_name, '/');
            if (lastSlash) {
                dirPath = std::string(dir_name, lastSlash - dir_name);
        ...
        int wd = inotify_add_watch(fd, dirPath.c_str(), mask);
        if (wd < 0) {
        ...
        while (true) {
            ...
            int pos = 0;
            while (pos < len) {
                const struct inotify_event *event = reinterpret_cast<const struct inotify_event *>(buf + pos);
                if (event->mask & mask) {
                    if (!fileName.empty()) {
                        if (event->len > 0 && strcmp(event->name, fileName.c_str()) == 0) {
    ...
    bool isAppInForegroundCpuset(const char* packageName) {
        static const char* cpusetPaths[] = {
            "/dev/cpuset/top-app/cgroup.procs",
            "/dev/cpuset/foreground/cgroup.procs"
        };
        for (int k = 0; k < 2; k++) {
            char procsBuf[1024] = {0};
            int fd = open(cpusetPaths[k], O_RDONLY);
            if (fd < 0) continue;
            ssize_t n = read(fd, procsBuf, sizeof(procsBuf) - 1);
            close(fd);
            if (n <= 0) continue;
            procsBuf[n] = 0;
            char* pidStr = strtok(procsBuf, " \n\t");
            while (pidStr) {
                int pid = atoi(pidStr);
                if (pid <= 0) { pidStr = strtok(nullptr, " \n\t"); continue; }
                char cmdlinePath[64];
                FastSnprintf(cmdlinePath, sizeof(cmdlinePath), "/proc/%d/cmdline", pid);
                char cmdline[256] = {0};
                int cfd = open(cmdlinePath, O_RDONLY);
                if (cfd < 0) { pidStr = strtok(nullptr, " \n\t"); continue; }
                ssize_t cn = read(cfd, cmdline, sizeof(cmdline) - 1);
                close(cfd);
                if (cn <= 0) { pidStr = strtok(nullptr, " \n\t"); continue; }
                cmdline[cn] = 0;
                if (strstr(cmdline, packageName) != nullptr) return true;
