...
public:
    ...
    void InotifyMain(const char* filePath, uint32_t mask, const char* watchFileName = nullptr) {
        std::string dirPath;
        std::string fileName;
        if (watchFileName) {
            dirPath = filePath;
            ...
            if (lastSlash) {
                dirPath = std::string(filePath, lastSlash - filePath);
        ...
        while (true) {
            ...
            while (pos < len) {
                ...
                if (event->mask & mask) {
                    if (event->len > 0 && !fileName.empty()) {
                        if (strcmp(event->name, fileName.c_str()) == 0) {
