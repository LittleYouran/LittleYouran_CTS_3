...
protected:
    ...
    alignas(Char) unsigned char _sso_buf[SSO_THRESHOLD * sizeof(Char)];
    bool _is_sso{true};
    ...
    INLINE constexpr void _init(Iter1 begin, Iter2 end) {
        ...
        if (size <= SSO_THRESHOLD) {
            _is_sso = true;
            copy(begin, end, reinterpret_cast<Char*>(_sso_buf));
            reinterpret_cast<Char*>(_sso_buf)[size] = '\0';
        } else {
            _is_sso = false;
...
public:
    ...
    INLINE constexpr explicit value(size_type capacity)
            : _impl{_allocator().template allocate<Char>(capacity + 1u)}, _size{0u},
              _capacity(capacity), _is_sso{false} {
        _impl[0] = '\0';
    }
    ...
    INLINE constexpr explicit value(size_type capacity, allocator_type& allocator)
            : base(allocator), _impl{_allocator().template allocate<Char>(capacity + 1u)},
              _size{0u}, _capacity(capacity), _is_sso{false} {
        _impl[0] = '\0';
    }
    ...
    INLINE constexpr value(self&& o) noexcept
            : base(move(o)), _is_sso(o._is_sso), _size(o._size), _capacity(o._capacity) {
        if (_is_sso) {
            copy(o.begin(), o.end(), reinterpret_cast<Char*>(_sso_buf));
            reinterpret_cast<Char*>(_sso_buf)[_size] = '\0';
        } else {
            _impl = o._impl;
        }
        o._impl = nullptr;
    ...
    NODISCARD INLINE constexpr const_pointer data() const noexcept { 
        if (_is_sso) {
            return reinterpret_cast<const_pointer>(_sso_buf);
        }
        return _impl; 
    }
    ...
    INLINE void emplace(Iter1 __begin, Iter2 __end) noexcept {
        ...
        if (_is_sso && __new_size <= SSO_THRESHOLD) {
            copy(__begin, __end, reinterpret_cast<Char*>(_sso_buf) + size());
            reinterpret_cast<Char*>(_sso_buf)[__new_size] = '\0';
            _size = __new_size;
            return;
        }
        
        if (_is_sso && __new_size > SSO_THRESHOLD) {
            Char* __new_impl = _allocator().template allocate<Char>(__new_size + 1u);
            copy(begin(), end(), __new_impl);
            copy(__begin, __end, __new_impl + size());
            __new_impl[__new_size] = '\0';
            
            _impl = __new_impl;
            _capacity = __new_size;
            _is_sso = false;
            _size = __new_size;
            return;
        }
        
        if (__new_size > _capacity) {
            size_type const __new_capacity = __new_size * 2;
            Char* __new_impl = _allocator().template allocate<Char>(__new_capacity + 1u);
            copy(begin(), end(), __new_impl);
            _allocator().template deallocate<Char>(_impl, _capacity);
    ...
    NODISCARD INLINE CONSTEXPR static enable_if_t<is_unsigned_v<_Tp>, self> from(_Tp __value) {
        auto __len = len(__value);
        self __str(__len);
        auto __ptr = __str.data();                   // ✅ 使用 data()
        __to_chars(__ptr + __len, __value);
        *(__ptr + __len) = '\0';
        __str._size = __len;
        return __str;
    }
