...
performance（性能模式开启了风驰）：应对风驰没有适配的游戏使用 没有限制频率等，不会按配置文件的参数执行。推荐'风驰没有适配的游戏使用'使用 <br>
fast（极速模式开启风驰/feas）：  经可能恢复官方风驰/feas调度 开启后不在按配置文件的参数执行。推荐'风驰适配了的游戏和要启用feas调度'使用<br>
performance（性能模式未开启风驰）：在保证较高流畅度的同时，可能会增加功耗。推荐'需要短时间性能的场景'使用 <br>
fast（极速模式未开启风驰）：全力保证游戏时的流畅度，忽略能效比，并没有任何'频率限制'并且所有核心将拉升至最高频率
...
Q: 调速器参数是什么东西?<br>
A: 这是Scheduler中的一些参数 这些设置将影响你的流畅度体验或续航 它的路径位于'/sys/devices/system/cpu/cpufreq/policyX/scheduler/' <br>

Q：为什么我在perapp_powermode设置了应用模式scene通知栏没有显示对应模式<br>
A：其实我也不知道 但以日志与实际应用是生效的，所以请以配置文件为准<br>

Q: scene和webui可以同时接管该调度吗？<br>
A： 当然是不可以的<br>

Q：恢复了官方调度但发现没有官方调度好用？<br>
A：与官方调度可能存在一些差异，但不会太大<br>

Q：恢复风驰调度可以使用风驰增强模块吗？<br>
A：没测过，但理论...emmmm...我也不知道（）<br>

Q: 其他问题怎么办？<br>
A：群里私聊或发到我的qq邮箱 （qq邮箱 3218633407@qq.com）<br>

##一些其他问题与说明
0. 使用scene 或者 webui 接管（二选一）
1. 不可以和其他调度一起使用
2. 可以使用线程模块
3. 使用优化模块自行排查是否冲突
4. 使用 cpu/gpu 等功能相关模块 自行排查是否冲突
5. 越狱不可以使用
4. 一般不需要使用其他模块进行干扰屏蔽官调模块
6. 8200使用了8100的配置文件是正常的
7. 骁龙8g2领先版使用了骁龙8g2的配置文件是正常的
8. 天玑800u使用天玑720配置文件是正常的
9. 官方未适配 fas  我也不打算适配
10. 如果刷入后 scene 未显示 重新覆盖刷入
11. ui 界面显示图标× 请看日志是否正常 正常就不用管 正常使用就好
12. 务必反馈功耗图的话  游戏 6-10 分钟  日常 1 小时左右或以上
13. 恢复了官方调度但与官方调度可能存在一些差异，但应该不会太大
14. 使用 perapp_powermode.txt 设置了 app 的使用模式但在 scene通知栏没有显示对应模式就以日志与具体实现为准

## 群聊与反馈
1群 545504856
2群 872472793
qq邮箱 3218633407@qq.com
...
| 字段名   | 数据类型 | 描述                                           |
| -------- | -------- | ---------------------------------------------- |
| enable | bool | feas官方调度开关 开启后极速模式将恢复调速器和频率修改 确保faes调度度能正常使用 |
...
```shell
echo "powersave" > /sdcard/Android/cts/config.txt
```
